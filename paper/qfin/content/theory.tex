...
Given the functions for the parameters $\alpha_t = f_{\alpha}(t, \textbf{P})$, $\rho_t = f_{\rho}(t, \textbf{Q})$, and $\nu_t = f_{\nu}(t, \textbf{R})$\footnote{
    We fix the value for $\beta$ to a constant and use it as hyperparameter instead,
    as it corresponds to the sensitivity of the forward price towards movements in the spot price and generally remains the same over time for each asset.
},
...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.7\textwidth]{images/unfitted}
        \caption{Surface generated using SABR models fit on some candidate set containing the complete daily observation set.}
        \label{fig:unfitted}
    \end{center}
\end{figure}
