...
The pricing equations for these derivatives, such as the Black-Scholes model~\cite{Black1973}, typically require knowing the time to maturity of the contract,
the price of the underlying asset to be traded, the market's current risk-free rate, and the price at which the asset will be executed at maturity, also called strike price~\cite{Hull2018}.
...
that is, the no-arbitrage price for the contract assuming an efficient market~\cite{Hull2018}.
...
Ensuring a way to obtain these values with confidence and using only readily available data is a crucial task for quantitative analysts~\cite{Kim2006}.
Usual strategies for estimating these values are predominantly based on linear or spline interpolation
algorithms~\cite{Homescu2011} or stochastic differential equation models~\cite{Jordan2011}
...
large losses of profit or even negative return~\cite{Hagan2014} given the leveraging capabilities of options when compared to non-derivative securities.
Classic models such as linear interpolation, polynomial fitting, and \textit{splines} also lack flexibility for extrapolation
and often generate either non-smooth surfaces, arbitrage opportunities, or both~\cite{Hagan2014}.
...
Instead of manually selecting these candidate points, models capable of automatically receive point clouds and return a numerical score to be used as selection criteria,
such as transformers, have been proposed~\cite{Cao2020, Kim2021, Bae2024, Jerbi2020}.
...
to be used together the transformer architecture~\cite{Vaswani2017} for the task of filtering candidate points for the \textit{SABR} model
This approach is an implicit formulation variation for the scoring model proposed by Kim et al. (2021)~\cite{Kim2021},
aiming to enforce a zero arbitrage condition in the SABR optimization process.
...
fitting tasks similar to the one presented in this paper~\cite{Kawaguchi2021},
all while being computationally faster for solving interpolation problems than linear layer models with similar performance~\cite{Kolter2020}.
Additionally, transformers have also been shown to perform better at handling unordered point clouds over time compared to recurrent neural layers~\cite{Kim2024}
...
optimizing the parameters for a domain-specific model in the context of IVS interpolation has been shown to be more effective~\cite{Almeida2023}
...
It measures how much removing a candidate point would affect the generated surface shape and
smoothness\footnote{The use of a per-point score is similar to what the principal components of a collection of points are when using the Principal Component Analysis (PCA) technique.}.
The highest scoring points --- which best represent the surface --- are then selected and used as input for the BFGS algorithm used to fit the \textit{SABR} model parameters.
This process aims to remove outlier points that might introduce variance, and consequently arbitrage opportunities~\cite{Kim2021},
...
Our proposed architecture applies the AA method as shown in \hyperref[alg:anderson]{Algorithm~\ref*{alg:anderson}}
...
The final deep equilibrium model used within the implicit layer (\hyperref[alg:forward]{Algorithm~\ref*{alg:forward}})
