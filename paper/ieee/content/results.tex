...
The dataset used for training was obtained from historical traded option contracts on the $SPY$ asset from January 2021 to December 2022, with a total of 68 unique days.
...
The target scores were generated beforehand using the aforementioned Monte Carlo method with $p = 80\%$ and $k = 1 \times 10^{2}$.
...
Our transformer model was trained on the dataset, with about 44k observations total resulting in 68 days.
With an early stopping sensitivity of $\epsilon = 10^{-3}$, the training was run with a maximum epoch range of $10^{3}$ using the Adam optimizer.
For the implicit layer, we used the Anderson Acceleration algorithm with a maximum of 10 iterations,
convergence threshold of $\eps=10^{-6}$, damping factor of $\lambda=0.1$, and memory size of $m=5$.
...
The network architecture is a transformer encoder block with 8 attention heads, 2 linear layers with 256 hidden units and 2 implicit layers with 256 and 128 hidden units, respectively.
...
For benchmarking, the execution time was measured using the Python \texttt{perf\_counter} method and a heatup of the model was executed on the entire dataset.
Separate tests using a linear interpolation with the original points and with the implicit layer were performed to compare visually the generated surface's error.
The training and tests were performed using leave-one-out cross-validation (LOO), due to the small amount of days in the dataset (68 days).
The train and test losses shown in \hyperref[tab:training]{Table~\ref*{tab:training}} are calculated using LOO,
while the execution time shown below in \hyperref[tab:performance]{Table~\ref*{tab:performance}} is calculated using the entire dataset.
...
and a visual comparison of the surfaces is shown in \hyperref[fig:figure4]{Figure~\ref*{fig:figure4}}.
